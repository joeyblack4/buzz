...
fn member_projection(record: &AgentDefinition) -> TeamCatalogMember {
    ...
    // a built-in would change its projection bytes and break the reuse-hint
    // hash, which must stay recomputable from the recipient's pristine local
    // copy.
    //
    // Non-built-in members: oversized inline raster data URLs are downscaled
    // so the share succeeds. An owner who set a large avatar should be able to
    // share — the catalog renders at small sizes anyway. If decoding fails or
    // no dimension fits, the avatar falls through unchanged and `validate_member`
    // surfaces the deterministic "avatar too large" error.
